...
/* Descricao: implementa  o problema dos leitores/escritores usando variaveis de condicao da biblioteca Pthread
  LABORATÓRIO 6
  VICTOR DA SILVA MOURA DRE: 120124419 
*/
...
int auxEscr = 0; //Auxiliar para a nova condição
...
void InicEscr (int id) {
   pthread_mutex_lock(&mutex);
   auxEscr++; // ESTÁ ESPERANDO PRA ESCREVER
   ...
   escr++;
   auxEscr--; //TERMINOU E O LEITOR PODE LER
   pthread_mutex_unlock(&mutex);
}
